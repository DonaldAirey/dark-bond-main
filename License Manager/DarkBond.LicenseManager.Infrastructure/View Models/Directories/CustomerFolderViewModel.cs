...
namespace DarkBond.LicenseManager.ViewModels.Directories
{
    ...
    /// </summary>
    public class CustomerFolderViewModel : CommonDirectoryViewModel
    {
        /// <summary>
        ...
        {
            // This will keep the view models of customers reconciled to the data model.
            this.DataModel.Customer.CollectionChanged += this.OnCollectionChanged;

            // This will initialize the collection of customers in this directory from the data model.  Building a view can be expensive, so disable
            // the automatic refreshes during the bulk operation.
            this.OnCollectionChanged(this, new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Add, this.DataModel.Customer));
        ...
        protected override ObservableCollection<IDisposable> CreateAppBarItems()
        {
            ...
            newCustomerButton.Command = new DelegateCommand(() => { this.LicenseService.NavigateToCustomer(); });
            ...
            using (this.Items.View.DeferRefresh())
            {
                ...
                switch (notifyCollectionChangedEventArgs.Action)
                {
                    case NotifyCollectionChangedAction.Reset:
                        ...
                        foreach (ListViews.CustomerViewModel countryViewModel in this.Items)
                        {
                    ...
                    case NotifyCollectionChangedAction.Add:
                        ...
                        foreach (CustomerRow customerRow in notifyCollectionChangedEventArgs.NewItems)
                        {
                            ...
                            ListViews.CustomerViewModel customerViewModel = this.CompositionContext.GetExport<ListViews.CustomerViewModel>();
                            customerViewModel.Map(customerRow);

                            // This will order the children so it's easy to find them individually and delete them.
                            int index = this.Items.BinarySearch((ivm) => ivm.SortKey, customerViewModel.CustomerId);
                    ...
                    case NotifyCollectionChangedAction.Remove:
                        ...
                        foreach (CustomerRow customerRow in notifyCollectionChangedEventArgs.OldItems)
                        {
                            ...
                            int index = this.Items.BinarySearch((ivm) => ivm.SortKey, customerRow[DataRowVersion.Original].CustomerId);
                            if (index >= 0)
                            {
