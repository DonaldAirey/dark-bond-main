// <copyright file="PersistentSingleRecordTests.cs" company="Dark Bond, Inc.">
...
namespace DarkBond.LicenseManager.DataModelTests
{
    ...
    [TestFixture]
    [Category("Persistent")]
    public sealed class PersistentSingleRecordTests : IDisposable
    {
        /// <summary>
        ...
        public void Initialize()
        {
            ...
            mockSettings.SetupGet(x => x.SqlConnectionString).Returns(PersistentSingleRecordTests.ConnectionString);
            ...
            using (SqlConnection sqlConnection = new SqlConnection(PersistentSingleRecordTests.ConnectionString))
            {
                // Open the connection.
                sqlConnection.Open();

                // Purge the Configuration table.
                using (SqlCommand sqlCommand = new SqlCommand("delete from [Configuration] where [ConfigurationId] in ('Alpha', 'Beta', 'Delta', 'Epsilon', 'Gamma', 'NewTest', 'Test', 'Zeta')", sqlConnection))
                {
                ...
                using (SqlCommand sqlCommand = new SqlCommand("delete from [Country] where [Abbreviation] in ('BS1', 'C', 'C1')", sqlConnection))
                {
                    sqlCommand.ExecuteNonQuery();
                }
            }
            ...
            {
                // This is a data model with a persistent store.  It allows for unit testing of the two-phase commit between the volatile storage and
                // the database.
        ...
        public void CreateCountryCommit()
        {
            ...
            using (TestTransaction testTransaction = new TestTransaction())
            {
                ...
                CountryRow countryRow = this.dataModel.CreateCountry("C1", countryId, "COUNTRY1", "Generic Country #1");
                ...
                Assert.That(this.dataModel.CountryExternalId0Key.Find("COUNTRY1"), Is.Not.Null);
        ...
        public void CreateCountryRollback()
        {
            ...
            {
                // Add the country record.
                CountryRow countryRow = this.dataModel.CreateCountry("C2", countryId, "COUNTRY2", "Generic Country #2");

                // Validate that the record was added to the unique index.
                Assert.That(countryRow, Is.Not.Null);
                Assert.That(countryRow.RowState, Is.EqualTo(RowState.Added));
                Assert.That(this.dataModel.CountryExternalId0Key.Find("COUNTRY2"), Is.Not.Null);
            ...
            using (TestTransaction testTransaction = new TestTransaction())
            {
                ...
                Assert.That(this.dataModel.CountryExternalId0Key.Find("COUNTRY2"), Is.Null);
            }
