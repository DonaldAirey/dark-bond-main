...
namespace DarkBond.SubscriptionManager.ViewModels
{
    ...
    using DarkBond.SubscriptionManager.ViewModels.Directories;
    ...
    public class FrameViewModel : FrameViewModelBase
    {
            ...
            : base(compositionContext, eventAggregator, navigationService)
        {
            ...
            this.directoryMap = new Dictionary<DirectoryTypes, Func<DirectoryViewModel>>
            {
                ...
                { DirectoryTypes.UnderwriterFolder, () => this.CompositionContext.GetExport<UnderwriterFolderViewModel>() },
                { DirectoryTypes.Customer, () => this.CompositionContext.GetExport<UnderwriterViewModel>() },
                { DirectoryTypes.OfferingFolder, () => this.CompositionContext.GetExport<OfferingFolderViewModel>() },
                { DirectoryTypes.Product, () => this.CompositionContext.GetExport<OfferingViewModel>() }
            };
        ...
        {
            NotValid, Application, UnderwriterFolder, Customer, OfferingFolder, Product
        }
            ...
            if (elements.Length >= 1)
            {
                if (elements[0] == Resources.ApplicationName)
                {
                    ...
                    else
                    {
                        ...
                        {
                            targetClass = elements.Length == 2 ? DirectoryTypes.UnderwriterFolder : DirectoryTypes.Customer;
                        }

                        if (elements[1] == Resources.Product)
                        {
                            targetClass = elements.Length == 2 ? DirectoryTypes.OfferingFolder : DirectoryTypes.Product;
                        }
                    }
                }
            }
