...
namespace DarkBond.Views.Controls
{
    ...
    public class ColumnViewDefinition : ViewDefinition
    {
        ...
        public ColumnViewDefinition()
        {
            ...
            this.Style = Application.Current.TryFindResource(new ComponentResourceKey(typeof(ItemsView), "ColumnViewStyle")) as Style;
        ...
        /// <summary>
        /// Gets or sets a value indicating whether columns in a ColumnViewDefinition can be reordered by a drag-and-drop operation.
        /// </summary>
        /// <value>
        /// A value indicating whether columns in a ColumnViewDefinition can be reordered by a drag-and-drop operation.
        /// </value>
        ...
        /// </summary>
        /// <value>
        /// The padding used for the column headers.
        /// </value>
        ...
        /// </summary>
        /// <value>
        /// The background brush used for the row items.
        /// </value>
        ...
        private static void AddColumns(ColumnViewDefinition columnView, NotifyCollectionChangedEventArgs notifyCollectionChangedEventArgs)
        {
            ...
            foreach (ColumnViewColumn columnViewColumn in notifyCollectionChangedEventArgs.NewItems)
            {
                ...
                // that value.
                if (!columnViewColumn.IsCellPaddingSet)
                {
                    Binding cellPadingBinding = new Binding
                    {
                        Path = new PropertyPath("CellPadding"),
                        Source = columnView
                    };
                    BindingOperations.SetBinding(columnViewColumn, ColumnViewColumn.CellPaddingProperty, cellPadingBinding);
                }

                // Same general logic with the header padding.  Use a common padding if the column hasn't explicitly overridden it.
                if (!columnViewColumn.IsHeaderPaddingSet)
                {
        ...
        private DataTemplate GenerateRowTemplate()
        {
            ...
            XNamespace controlsNamespace = "clr-namespace:DarkBond.Views.Controls;assembly=DarkBond.Views";
            ...
            // This compiled DataTemplate will create rows much faster than a design that attempts to do the same thing programmatically as the
            // rendering engine handles a tree built from the top down much better than a visual tree built from the bottom (the cell controls) up.
            DataTemplate dataTemplate = null;
            using (MemoryStream memoryStream = new MemoryStream(Encoding.ASCII.GetBytes(xDocument.ToString())))
            {
                dataTemplate = XamlReader.Load(memoryStream) as DataTemplate;
            }
        ...
        private void OnColumnDragStarted(object sender, EventArgs eventArgs)
        {
            ...
            if (this.AllowsColumnReorder)
            {
                ...
                // This will place the header behind all the other headers as it is dragged around.  Since the headers are normally transparent, this
                // will give the effect of the two columns superimposed on each other.
                ...
                if (currentColumnIndex != this.destinationColumnIndex && currentColumnIndex != this.draggingColumnIndex)
                {
                    ...
                    for (int columnIndex = 0; columnIndex < this.Columns.Count; columnIndex++)
                    {
                        ...
                        if (columnViewColumn.DesiredLeft != desiredLeft)
                        {
                            ...
                            Storyboard.SetTargetProperty(doubleAnimation, new PropertyPath("Left"));
        ...
        private void OnSortChanged(object sender, SortChangedEventArgs sortChangedEventArgs)
        {
            ...
            {
                // If this is not an extended search, than remove all the previous fields from the search.
                if (!sortChangedEventArgs.IsExtended)
                {
                    while (this.sortColumns.Count > 1)
                    {
                        this.sortColumns.RemoveAt(0);
                    }
                }

                // Toggle the search direction of this column.
